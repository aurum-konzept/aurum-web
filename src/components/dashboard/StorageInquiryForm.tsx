...
type Metal = "Silber" | "Gold" | "Platin" | "Palladium";
type Unit = "g" | "kg" | "oz";

type Item = {
  metal: Metal;
  amount: string; // string -> wir validieren beim Submit
  unit: Unit;
};

const metalOptions: Metal[] = ["Silber", "Gold", "Platin", "Palladium"];
    ...
    try {
      const res = await fetch("/api/inquiry/storage", {
        ...
        body: JSON.stringify({
          ...
          note,
          interestFuture,
          items: cleaned.map(({ metal, unit, amountNum }) => ({ metal, unit, amount: amountNum })),
        }),
      });
    ...
    <form onSubmit={submit} className="space-y-4">
          ...
          {items.map((it, idx) => (
            <div key={idx} className="grid gap-3 md:grid-cols-12 items-center">
              <div className="md:col-span-4">
                <select
                  value={it.metal}
                  onChange={(e) => updateItem(idx, { metal: e.target.value as Metal })}
              ...
              <div className="md:col-span-1 flex justify-end">
                {items.length > 1 && (
                  <button
                    ...
                    className="text-sm text-neutral-400 hover:text-red-300 transition"
                    title="Entfernen"
                  >
                    ✕
                  </button>
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-950/40 p-4">
        <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="text-sm text-neutral-300 leading-relaxed">
                ...
                <span className="text-amber-200">Aurum</span>{" "}
                verwahren und Ihr Vermögen nutzen – statt verkaufen?
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="z.B. Verpackung, Abholung, gewünschte Laufzeit, besondere Anforderungen ..."
        />
      </div>
